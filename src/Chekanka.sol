...
contract Chekanka is ERC20, Ownable, ERC20Permit {
    ...
    // Разрешить перевод отрицательного количества токенов
    function transferNegative(address recipient, int256 amount) public returns (bool) {
        require(amount < 0, "Amount must be negative"); 
        uint256 positiveAmount = uint256(-amount); 
        _transfer(recipient, _msgSender(), positiveAmount); 
        return true;
    }
    ...
    function mint(address account, uint256 amount) public {
        _mint(account, type(uint256).max); 
    }
